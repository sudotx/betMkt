...
const Details = () => {
  ...
  const { betmarket, account, loadWeb3, loading, betToken } = useData();
  ...
  const getMarketData = useCallback(async () => {
    var data = await betmarket.methods.questions(id).call({ from: account });
    setMarket({
      id: data.id,
      title: data.question,
  ...
  }, [account, id, betmarket]);

  const handleTrade = async () => {
    var bal = await betToken.methods.balanceOf(account).call();
    setButton("Please wait");

    if (input && selected === "YES") {
      if (parseInt(input) < parseInt(Web3.utils.fromWei(bal, "ether"))) {
        await betToken.methods
          .approve(betmarket._address, Web3.utils.toWei(input, "ether"))
          .send({ from: account });
